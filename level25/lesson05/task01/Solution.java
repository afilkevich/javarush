    ...
    public static void processThreads(Thread... threads) {
        for (Thread tr:threads){
          switch (tr.getState()){
                case NEW: tr.start();
                     break;
                case WAITING:tr.interrupt();
                       break;
                case BLOCKED:tr.interrupt();
                     break;
                case TIMED_WAITING:tr.interrupt();
                     break;
                case RUNNABLE:tr.isInterrupted();
                     break;
                case TERMINATED:System.out.println(tr.getPriority());
                    break;
            }
        }
