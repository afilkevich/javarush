    ...
    public Sayable toSayable(final int i) {
        class DogPet extends SuperDog implements Sayable{
            private String getName(){
                return new StringBuilder(getSuperQuotes()).append(name).append(getSuperQuotes()).toString();
            }
            ...
            public String say()
            {
                ...
                if (i<=0){
                    ...
                    build.append(" лает г");
                    for (int j=0;j<i;j++){build.append("а"); }
                    build.append("в! ");
                    build.append(formatter.format(new Date()));
