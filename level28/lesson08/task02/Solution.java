...
public class Solution {
    public static void main(String[] args) throws InterruptedException {
        LinkedBlockingQueue<Runnable> runnableLinkedBlock=new LinkedBlockingQueue<>();
        for (int i=1;i<=10;i++){
            final int finalI = i;
            runnableLinkedBlock.offer(new Runnable()
            {
                @Override
                public void run()
                {
        ...
        ThreadPoolExecutor threadPool=new ThreadPoolExecutor(3,5,1000,TimeUnit.MILLISECONDS,runnableLinkedBlock);
